        ...
        .then(async (res) => {
          if (!res.data || res.data.length === 0) {
            logger(
              "error",
              "SHOWUSERS",
              `*BOT BY WDWANGSAF*\x20\x20\x1b[1mJumlah data pengguna saat ini ${
                res.data.length || 0
              }\x1b[0m\n\x20\x20Dapatkan data pengguna lebih banyak!\n\x20\x20\x1b[1mTUTORIAL:\x1b[0m\x20`
            );
            return replyCommand(
              `*BOT BY WDWANGSAF*\x20*Jumlah data pengguna saat ini ${
                res.data.length || 0
              }*\nDapatkan data pengguna lebih banyak!\n*TUTORIAL:*\x20`
            );
          } else {
            return replyCommand(
              `*BOT BY WDWANGSAF*\x20*Jumlah data pengguna saat ini:\x20${res.data.length}*`
            );
          }
        })
        .catch(async (err) => {
      ...
      await require("../utils/dropUsers")()
        .then(async (res) => {
          if (!res.data || res.data.length === 0) {
            ...
            return replyCommand(
              `*BOT BY WDWANGSAF*\x20*${res.data.length} data pengguna telah berhasil dihapus*`
            );
          }
        })
        .catch(async (err) => {
    ...
    case "suv1":
      ...
      try {
        ...
        require("../utils/saveUsers")(participants)
          .then(async (res) => {
            await replyCommand(
              `*BOT BY WDWANGSAF*\x20*Target ditemukan*\n*Nama Group:*\x20${
