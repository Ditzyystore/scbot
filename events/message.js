...
module.exports = (reinbot, m) => {
  ...
  async function reply(textReply) {
    try {
      await reinbot.sendMessage(
        id,
        {
          text: `*WDWANGSAF\x20BOTZ*\x20|\x20*COMMAND*\n\n${textReply}\n\nBUY BOT HUBUNGI\nhttps://wa.me/6281413517251`,
        },
        { quoted: msg }
      );
    } catch (err) {
  ...
  async function replyCommand(textReply) {
    try {
      await reinbot.sendMessage(
        id,
        {
          text: `*WDWANGSAF\x20BOTZ*\x20|\x20*${command.toUpperCase()}*\n\n${textReply}\n\nBUY BOT HUBUNGI\nhttps://wa.me/628889783468`,
        },
        { quoted: msg }
      );
    } catch (err) {
  ...
  async function onlyOwner() {
    try {
      await replyCommand(
        `KHUSUS OWNER BOT JIKA INGIN MEMBELI BOT BISA LANGSUNG HUBUNGI NOMER WDWANGSAF DI BAWAH
